...
def add_me_to_the_queue(
    ...
    :param ticket_type: int: int - type of ticket. 1 = express, 0 = normal.
    :param person_name: str - name of person to add to a queue.
    :return: list - the (updated) queue the name was added to.
    """

    return (
        concat(express_queue, [person_name])
        if bool(ticket_type)
        else concat(normal_queue, [person_name])
    )
...
def add_me_with_my_friends(queue: List[str], index, person_name: str):
