...
function* primeGenerator(): Generator<number, never, undefined> {
    // Start with the first three primes (basis for wheel)
    const foundPrimes = [2, 3, 5];
    yield* foundPrimes;

    let candidate = 7; // First candidate after wheel basis
    let wheelPosition = 0;
...
function memoize<T>(gen: Generator<T, never, undefined>): (idx: number) => T {
    ...
    return (idx: number): T => {
        ...
        // Generate primes up to requested index
        while (cache.length <= idx) {
            cache.push(gen.next().value);
        }
        return cache[idx];
    };
}
