...
abstract class Planet {
    static readonly EARTH_YEAR_IN_SECS: number = 365.25 * 24.0 * 60.0 * 60.0;
    abstract readonly ORBITAL_PERIOD: number;

    from(duration: Duration): number {
        return Number(
            (
                duration.seconds /
                (Planet.EARTH_YEAR_IN_SECS * this.ORBITAL_PERIOD)
            ).toFixed(2),
        );
    }
}
...
export function age(planetName: string, seconds: number): number {
    ...
    return planet.from(new Duration(seconds));
}
